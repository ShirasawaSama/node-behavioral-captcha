...
import { type Config, defaultConfig, type Size, type Range } from './config.js'
// import { writeFileSync } from 'fs'
import { drawCircles, drawRandomLines } from './utils.js'

const createGenerator = async (config: Partial<Config> = { }): Promise<Captcha> => {
  ...
  return new Captcha(cfg, await Promise.all(cfg.backgroundImages.map(async it => await loadImage(it))))
}
